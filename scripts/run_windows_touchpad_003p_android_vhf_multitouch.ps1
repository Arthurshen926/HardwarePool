...
$evidencePath = Join-Path $evidenceRoot 'CAPY-PTP-003P-android-vhf-multitouch.txt'

$identity = [Security.Principal.WindowsIdentity]::GetCurrent()
$principal = [Security.Principal.WindowsPrincipal]::new($identity)
if (-not $principal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
    throw 'The Android-to-VHF multitouch acceptance requires an elevated administrator PowerShell.'
}
...
try {
    Write-Output 'acceptance=CAPY-PTP-003P'
    ...
    if ($labExit -ne 0) {
        throw "Android-to-VHF multitouch acceptance failed: $labExit"
    }
    ...
    if (-not $maxMatch.Success) {
        throw 'Android-to-VHF output did not report max_contacts_observed.'
    }
    $maxContacts = [int]$maxMatch.Groups[1].Value
    if ($maxContacts -lt 4) {
        Write-Output "acceptance_status=incomplete; max_contacts=$maxContacts; required_contacts=4"
        throw "Android-to-VHF run observed only $maxContacts contacts; four are required."
    }
    Write-Output "CAPY-PTP-003P Android-to-VHF physical multitouch: PASS; max_contacts=$maxContacts"
}
finally {
    Stop-Transcript
}
