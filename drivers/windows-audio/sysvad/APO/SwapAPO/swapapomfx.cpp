...
{
    // CapyIO's bridge never changes the stream format. The inherited SysVAD
    // sample advertised optional custom stereo formats and then attempted to
    // negotiate them for both the mono capture and stereo ingress graphs. That
    // prevents the Audio Engine from producing a mix format for either graph.
    // Keep negotiation identical to the Windows base APO contract and perform
    // only the bounded ring copy in APOProcess.
