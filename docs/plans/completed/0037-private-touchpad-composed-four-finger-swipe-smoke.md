# CAPY-PTP-002M — Composed four-finger swipe native smoke
...
With separate human authorization, submit the fixed closed four-contact
...
- the existing deterministic four-finger fixture: CancelAll, eight gradual
  updates, empty release and final CancelAll;
- stable contact IDs 1..4, x movement from 2500 to 7500 and fixed y positions
  1500, 2500, 3500 and 4500;
- 15 ms submission intervals, exact packet metrics and full cleanup.

## Out of scope

- arbitrary or user-provided gestures;
- claiming visible Windows system behavior from API success alone;
...
2. Four contact IDs remain stable across all eight update frames.
...
- Default worker tests report eight passed and five ignored.
- Targeted Clippy and full default repository CI pass without desktop-input
  execution. The first sandboxed CI attempt failed only because two unrelated
  Audio Share tests could not bind local TCP sockets (Windows 10013); the same
  CI passed outside that socket restriction.
- The authorized exact test reports one passed and twelve filtered.
- Eleven packets were submitted, the four contacts were released and
  cancelled, then Sink/device and Runtime cleanup completed.

Detailed evidence: `docs/CAPY_PTP_002M_REPORT.md`.
