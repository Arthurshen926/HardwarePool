...
/** Closed trusted-lab configuration for one native microphone Route epoch. */
public final class NativeLanMicrophoneSessionConfig {
    ...
    public NativeLanMicrophoneSessionConfig(
            String peerIpv4,
            int localPort,
            int peerPort,
            UUID sessionId,
        ...
        if (localPort < 1 || localPort > 65_535 || peerPort < 1 || peerPort > 65_535) {
            throw new IllegalArgumentException("microphone ports must be non-zero u16 values");
        }
        ...
        if (peer.isAnyLocalAddress()
            ...
            throw new IllegalArgumentException("microphone peer must be a concrete unicast IPv4");
        }
    }

    private static InetAddress ipv4Literal(String value) {
        if (value == null || value.isEmpty() || value.length() > 15) {
            throw new IllegalArgumentException("microphone address must be an IPv4 literal");
        }
