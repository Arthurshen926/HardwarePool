...
impl ProfileId {
    ...
    pub fn audio_capture_v1() -> Self {
        Self::new("hardwarepool.audio.capture", 1)
    }

    #[must_use]
    pub fn audio_render_v1() -> Self {
        Self::new("hardwarepool.audio.render", 1)
    }

    #[must_use]
    pub fn audio_duplex_bundle_v1() -> Self {
        Self::new("hardwarepool.audio.duplex_bundle", 1)
    }
    ...
    pub fn validate(&self) -> Result<(), CoreError> {
        ...
        match (&self.kind, &self.details) {
            (CapabilityKind::AudioCapture, CapabilityDetails::Audio(spec)) => {
                self.validate_profile("hardwarepool.audio.capture")?;
            ...
            (CapabilityKind::AudioRender, CapabilityDetails::Audio(spec)) => {
                self.validate_profile("hardwarepool.audio.render")?;
            ...
            (CapabilityKind::AudioDuplexBundle, CapabilityDetails::AudioDuplexBundle(bundle)) => {
                self.validate_profile("hardwarepool.audio.duplex_bundle")?;
...
/// Static description of a HardwarePool runtime instance.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
